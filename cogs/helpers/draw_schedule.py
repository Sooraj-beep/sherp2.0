...
def draw_schedule(sched):
    ...
    for course_class in classes:
        ...
        for classtime in course_class["classtimes"]:
            ...
            for day in days:
                ...
                r_y0 = (
                    top_margin_offset + quarters_past * 25.25 + (quarters_past / 4) * 3
                )
                r_y1 = r_y0 + quarters_fill * 25.25 + (quarters_fill / 4 - 1) * 3

                draw.rectangle(
                    [(r_x0 - 2, r_y0 - 2), (r_x1 + 2, r_y1 + 2)], fill=(0, 0, 0)
                )
                draw.rectangle([(r_x0, r_y0), (r_x1, r_y1)], fill=color)
                location = location if location else "TBD"
                draw.text(
                    (r_x0 + 4, r_y0 + 2),
                    get_draw_text(course_class, location=location),
                    (0, 0, 0),
                    font=__draw_sched_font,
                )
    ...
    y_region_bottom = (
        top_margin_offset + bottom_hours * vertical_length_50 + bottom_hours * 3
    )
    ...
    image.paste(
        y_region,
        (0, top_margin_offset, boilerplate_width, top_margin_offset + y_region_length),
    )
    y_crop_line = top_margin_offset + y_region_length
    draw.line(
        (0, y_crop_line - 2, boilerplate_width, y_crop_line - 2),
        fill=(0, 0, 0),
        width=2,
    )
    ...
    if not class_on_weekend:
        ...
        image.paste(
            x_region,
            (left_margin_offset, 0, left_margin_offset + x_region_length, y_crop_line),
        )
