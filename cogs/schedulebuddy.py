...
class ScheduleSession:
    ...
    async def fetch_schedules(self, intr: discord.Interaction):
        courses = "[" + ",".join(map(lambda s: s.upper(), filter(lambda s: s, self.courses))) + "]"
        print(courses)
        url = SCHEDUBUDDY_ROOT + "gen-schedules/" 
        params = {"term": self.term_id,
                  "courses": courses,
        ...
        async with self.http.get(url, params=params) as resp:
            print(str(resp.url))
            if resp.status != 200:
                await intr.response.send_message("Schedule buddy returned non 200 response")
                return False
    ...
    def build_embed(self, display_name, schedule) -> Embed:
        embed = Embed(description=f'Listing **{self.current_page+1}**\
            of **{len(self.pages)}** pages', color=0xb3edbd)
...
class ScheduleBuddy(commands.GroupCog, name="schedubuddy"):
    ...
    @app_commands.command(name="view-schedule", 
    ...
    async def view(self, intr: discord.Interaction,
        ...
        async with self.http.get(url, params=params) as resp:
            if resp.status != 200:
                await intr.response.send_message("Error fetching data from schedulebuddy")
                return
    ...
    @app_commands.command(name="create-schedule", 
    ...
    @app_commands.describe(course1="Course code of the course you want to schedule. Eg \"CMPUT 174\". The space is important.")
    @app_commands.choices()
    @app_commands.choices()
    async def create_schedule(self, intr: discord.Interaction,
                    year: app_commands.Choice[str],
                    term: app_commands.Choice[str],
                    course1: str,
        ...
        term_id = self.get_term_id(year, term)
        session = ScheduleSession(self.__bot,
                        term_id,
                        start_time,
                        consec_pref,
                        evening_pref,
...
async def setup_schedule_buddy(bot, guilds):
    cog = ScheduleBuddy(bot)
    await cog.init_http_client()
